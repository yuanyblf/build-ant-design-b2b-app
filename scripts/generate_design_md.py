...
- 语言：{product['locale']}；密度：{product['density']}；内容最大宽度：{product['contentMaxWidth']}px。
...
- 表格不展示独立标题，操作按钮位于表格顶部左侧。每页默认 {table['pageSize']} 条，操作列固定：{yn(table['fixedActionColumn'])}。
- 可跳转内容使用链接。长内容默认完整展示；只有业务明确要求时才省略并用 Tooltip 展示全文。
{tree_rule}
## 表单、详情与弹窗

- 桌面端表单固定为横向布局：标签宽度 {patterns['formLabelWidth']}px、不换行，控件填满剩余空间；窄屏时整张表单统一切换为纵向。
...
- 弹窗字段必须提供说明并标注必填项；Footer 顺序为“{modal['confirmText']}”“{modal['cancelText']}”，不得使用 Ant Design 默认的反向顺序。

## 按钮与反馈

- 所有按钮组按主次从左到右排列，主操作位于最左侧；每个功能模块最多 {components['primaryButtonMaxPerModule']} 个主按钮。
