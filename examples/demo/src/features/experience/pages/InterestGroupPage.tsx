...
export function InterestGroupPage() {
  ...
  return <div className="page-stack">
    ...
    <Card><div className="table-toolbar"><Space><Button onClick={() => message.info('Demo：分类管理')}>管理分类</Button><Button type="primary" icon={<PlusOutlined />} onClick={() => setOpen(true)}>创建小组</Button></Space></div><Row gutter={[20,20]}>
    ...
    <Modal title="创建兴趣小组" open={open} onCancel={() => setOpen(false)} onOk={create} okText="确认" cancelText="取消"><Form form={form} layout="horizontal" className="edit-form"><Form.Item name="name" label="小组名称" extra="建议使用清晰、便于员工搜索的名称。" rules={[{ required: true, message: '请输入小组名称' }]}><Input /></Form.Item><Form.Item name="category" label="所属分类" extra="分类决定小组在左侧树中的位置。" rules={[{ required: true, message: '请选择分类' }]}><Select options={['运动健康','文化艺术','成长学习'].map(value => ({ value, label: value }))}/></Form.Item><Form.Item name="description" label="小组简介" extra="用于向员工说明小组主题和参与方式。" rules={[{ required: true, message: '请输入小组简介' }]}><Input.TextArea rows={3} maxLength={120} showCount /></Form.Item></Form></Modal>
  </div>;
}
