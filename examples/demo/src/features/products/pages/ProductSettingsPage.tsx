...
export function ProductSettingsPage({ mode }: { mode: 'category' | 'specification' }) {
  ...
  return <div className="page-stack">
    ...
    <SearchPanel onSearch={() => message.success('查询完成')} onReset={() => setKeyword('')}><SearchField label={`${noun}名称`}><Input allowClear placeholder={`请输入${noun}名称或编码`} value={keyword} onChange={event => setKeyword(event.target.value)} /></SearchField></SearchPanel>
    <Card><div className="table-toolbar"><Button type="primary" icon={<PlusOutlined />} onClick={() => edit()}>新增{noun}</Button></div><Table rowKey="id" dataSource={filtered} pagination={false} columns={[{ title: `${noun}名称`, dataIndex: 'name' }, { title: `${noun}编码`, dataIndex: 'code' }, { title: '关联商品数', dataIndex: 'count', align: 'right' }, { title: '状态', dataIndex: 'status', render: value => <Tag color={value === '启用' ? 'success' : 'default'}>{value}</Tag> }, { title: '操作', align: 'right', render: (_, record: SettingItem) => <Space><Button type="link" onClick={() => edit(record)}>编辑</Button><Popconfirm title={`确认${record.status === '启用' ? '停用' : '启用'}“${record.name}”？`} description={record.count ? `当前关联 ${record.count} 个商品，已有商品数据不会被删除。` : undefined} onConfirm={() => { setData(list => list.map(item => item.id === record.id ? { ...item, status: record.status === '启用' ? '停用' : '启用' } : item)); message.success('状态已更新'); }}><Button type="link">{record.status === '启用' ? '停用' : '启用'}</Button></Popconfirm></Space> }]}/></Card>
