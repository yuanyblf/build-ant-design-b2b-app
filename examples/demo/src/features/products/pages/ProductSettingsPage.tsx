...
export function ProductSettingsPage({ mode }: { mode: 'category' | 'specification' }) {
  ...
  return <div className="page-stack">
    ...
    <Modal title={`${editing ? '编辑' : '新增'}${noun}`} open={open} onCancel={() => setOpen(false)} onOk={save} okText="确认" cancelText="取消" footer={(_, { OkBtn, CancelBtn }) => <Space><OkBtn /><CancelBtn /></Space>}><Form form={form} layout="horizontal" className="edit-form"><Form.Item name="name" label={`${noun}名称`} extra={`用于商品维护时识别该${noun}。`} rules={[{ required: true, message: `请输入${noun}名称` }]}><Input /></Form.Item><Form.Item name="code" label={`${noun}编码`} extra="编码保存后用于数据关联，请保持唯一。" rules={[{ required: true, message: `请输入${noun}编码` }]}><Input /></Form.Item></Form></Modal>
  </div>;
}
